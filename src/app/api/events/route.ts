...
const DEFAULT_FORWARD_TO = "oyedejisamuel05@gmail.com";
...
/** Health check — open GET /api/events in the browser or curl to confirm the route is live. */
export function GET() {
...
export async function POST(request: NextRequest) {
  ...
  console.log("[api/events] POST", JSON.stringify(event, null, 2));
  ...
  if (!apiKey || !from) {
    console.error(
      "[api/events] Missing RESEND_API_KEY or RESEND_FROM (verified sender domain in Resend).",
    );
    return NextResponse.json(
      {
        error:
          "Server not configured: set RESEND_API_KEY and RESEND_FROM in the environment.",
      },
      { status: 503 },
    );
  }
  ...
  if (fetchError || !inbound) {
    console.error("[api/events] receiving.get", fetchError);
    return NextResponse.json(
  ...
  if (sendError || !sent) {
    console.error("[api/events] emails.send", sendError);
    return NextResponse.json(
